...
// Carregue as variáveis de ambiente no início da sua aplicação (ex: require('dotenv').config())
// A aplicação não deve iniciar sem uma chave secreta definida.
...
class UserController {
  ...
  async auth(req, res) {
    ...
    if (!JWT_SECRET) {
      console.error(
        "ERRO CRÍTICO: JWT_SECRET não está configurada no ambiente."
      );
      return res
        .status(500)
  ...
  static getToken(token) {
    try {
      if (!JWT_SECRET) {
        console.error(
          "ERRO CRÍTICO: JWT_SECRET não está configurada no ambiente para verificação."
        );
        return null;
      }
